...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $checkSql = "SELECT * FROM userr WHERE username='$username' OR email='$email'";
    $checkResult = $co   ->query($checkSql);

    if ($checkResult->num_rows > 0) {
        echo "<script>alert('Username atau email sudah terdaftar. Silakan gunakan yang lain.'); window.history.back();</script>";
    } else {
        // Menyiapkan dan mengeksekusi query
        $sql = "INSERT INTO userr (username, email, password) VALUES ('$username', '$email', '$password')";
